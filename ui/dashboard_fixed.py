...
def render_dashboard():
        ...
        if st.session_state.agent_logs:
            ...
            for log in st.session_state.agent_logs[-10:]:  # Show last 10 logs
                with st.container():
                    ...
                    html_content = f"""<div style='padding: 10px; border-left: 5px solid {status_color};'>
                    <strong>{log['timestamp'].strftime('%H:%M:%S')}</strong> - <strong>{log['agent']}</strong><br/>
                    {log['action']}<br/>
                    <em>Duration: {log['duration']:.2f}s</em>
                    </div>"""
                    st.markdown(html_content, unsafe_allow_html=True)
                    st.markdown("---")
        else:
            st.info("No agent activity recorded yet. Select an example from the sidebar to begin.")
    ...
    for edge in edges:
        ...
        fig.add_trace(go.Scatter(
            ...
            mode="lines",
            line=dict(width=2, color=edge.get("color", "gray"), dash=edge.get("dash", "solid")),
            hoverinfo="none",
            showlegend=False
        ))
    ...
    with st.expander("Workflow Explanation"):
        ...
        systems = [
            {"Component": "Email Processing", "Cloud Cost": "$50-100/month", "On-Prem Cost": "$500-1000 setup + maintenance"},
            {"Component": "AI Processing", "Cloud Cost": "$200-500/month", "On-Prem Cost": "$2000-5000 setup + maintenance"},
            {"Component": "Data Storage", "Cloud Cost": "$100-300/month", "On-Prem Cost": "$1000-3000 setup + maintenance"}
        ]
...
<strong>From:</strong> {file['sender']} &lt;{sender_email}@example.com&gt;<br/>
<strong>To:</strong> Data Processing Team &lt;data.processing@ourcompany.com&gt;<br/>
<strong>Subject:</strong> {file['subject']}<br/>
<strong>Date:</strong> {file['received_time'].strftime('%a, %d %b %Y %H:%M:%S')}<br/>
<strong>Attachments:</strong> {file['filename']}<br/>
<hr/>
<p>Hello Data Processing Team,</p>
<p>Please find attached the latest data file for processing.</p>
<p>We need this processed as soon as possible for our monthly reporting.</p>
<p>Thank you for your assistance.</p>
<p>Best regards,<br/>{file['sender']}</p>
</div>"""
                    st.markdown(html_content, unsafe_allow_html=True)
                ...
                if file_questions:
                    with st.expander("Outgoing Email - Clarification Questions", expanded=True):
                        ...
                        html_content = f"""<div style='border: 1px solid #ddd; padding: 15px; border-radius: 5px;'>
<strong>From:</strong> Data Processing Team &lt;data.processing@ourcompany.com&gt;<br/>
<strong>To:</strong> {file['sender']} &lt;{sender_email}@example.com&gt;<br/>
<strong>Subject:</strong> Re: {file['subject']} - Clarification Needed<br/>
<strong>Date:</strong> {(file['received_time'] + timedelta(minutes=5)).strftime('%a, %d %b %Y %H:%M:%S')}<br/>
<hr/>
...
<strong>From:</strong> Data Processing Team &lt;data.processing@ourcompany.com&gt;<br/>
<strong>To:</strong> {file['sender']} &lt;{sender_email}@example.com&gt;<br/>
<strong>Subject:</strong> Re: {file['subject']} - Processing Complete<br/>
<strong>Date:</strong> {(file['received_time'] + timedelta(minutes=15)).strftime('%a, %d %b %Y %H:%M:%S')}<br/>
<hr/>
<p>Hello {sender_first_name},</p>
<p>We have successfully processed the data file you sent ({file['filename']}).</p>
<p>The data has been transformed into our standard format and loaded into the following systems:</p>
<ul>
    <li>Core Transaction System</li>
    <li>Data Warehouse</li>
</ul>
<p>Processing Summary:</p>
<ul>
    <li>Records Processed: {random.randint(10, 100)}</li>
    <li>Processing Time: {file['processing_time']:.2f} seconds</li>
    <li>Validation Status: Passed</li>
</ul>
